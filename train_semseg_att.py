...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='pointnet2_att_Ablation1', help='Log path [default: None]')
...
def main(args):
    ...
    TRAIN_DATASET = S3DISDatasetTrans(split='train', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=5.0, shuffle_index=True, transform=train_transform)
    print("start loading test data ...")
    TEST_DATASET = S3DISDatasetTrans(split='test', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=5.0, shuffle_index=False, transform=None)
        ...
        with torch.no_grad():
            ...
            for l in range(NUM_CLASSES):
                iou_per_class_str += 'class %s weight: %.3f, IoU: %.3f \n' % (
                    seg_label_to_cat[l] + ' ' * (13 + 1 - len(seg_label_to_cat[l])), labelweights[l - 1],
