...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='pointnet2_att_trans', help='Log path [default: None]')
    parser.add_argument('--decay_rate', type=float, default=1e-4, help='weight decay [default: 1e-4]')
    parser.add_argument('--npoint', type=int, default=20000, help='Point Number [default: 4096]')
    parser.add_argument('--step_size', type=int, default=10, help='Decay step for lr decay [default: every 10 epochs]')
    parser.add_argument('--lr_decay', type=float, default=0.1, help='Decay rate for lr decay [default: 0.7]')
...
def main(args):
    ...
    TRAIN_DATASET = S3DISDatasetTrans(split='train', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=50.0, shuffle_index=True, transform=train_transform)
    print("start loading test data ...")
    TEST_DATASET = S3DISDatasetTrans(split='test', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=10.0, shuffle_index=False, transform=None)
    ...
    try:
        ...
        if 'craslab3d' in root:
            ...
            pretrained_state_dict = {k: v for k, v in checkpoint['model_state_dict'].items() if k in model_state_dict and 'cls' not in k}
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        loop = tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), leave=False, smoothing=0.9)
        loop.set_description('train')

        for i, (coord, feat, target, offset) in loop:
            optimizer.zero_grad()

            coord, feat, target, offset = coord.cuda(non_blocking=True), feat.cuda(non_blocking=True), target.cuda(non_blocking=True), offset.cuda(non_blocking=True)

            seg_pred, trans_feat = classifier([coord, feat, offset])
        ...
        with torch.no_grad():
            ...
            for i, (coord, feat, target, offset) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                coord, feat, target, offset = coord.cuda(non_blocking=True), feat.cuda(non_blocking=True), target.cuda(non_blocking=True), offset.cuda(non_blocking=True)
                ...
                pred_val = pred_val[batch_label!=-1]
                batch_label = batch_label[batch_label!=-1]
            ...
            for l in range(NUM_CLASSES):
                iou_per_class_str += 'class %s weight: %.3f, IoU: %.3f \n' % (
                    seg_label_to_cat[l] + ' ' * (18 + 1 - len(seg_label_to_cat[l])), labelweights[l - 1],
