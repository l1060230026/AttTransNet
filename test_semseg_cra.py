...
def parse_args():
    ...
    parser.add_argument('--num_point', type=int, default=25000, help='point number [default: 4096]')
    parser.add_argument('--voxel_size', type=float, default=0.04, help='voxel size')
    parser.add_argument('--log_dir', type=str, default='pointnet2_att_cra', help='experiment root')
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            ...
            # ground_truth.append(whole_scene_label)
            # unique = np.setdiff1d(unique, np.intersect1d(np.unique(whole_scene_label), unique))
            # if len(unique) == 0:
            #     break
