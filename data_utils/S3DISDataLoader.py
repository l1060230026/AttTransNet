...
from data_utils.data_util import sa_create
...
class S3DISDatasetTrans(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0, sample_rate=1.0, voxel_size=0.04, shuffle_index=False, transform=None):
        super().__init__()
        self.split, self.voxel_size, self.transform, self.voxel_max, self.shuffle_index = split, voxel_size, transform, num_point, shuffle_index
        self.num_point = num_point
        self.block_size = block_size
        self.transform = transform
        num_class = 13
        ...
        for room_name in tqdm(rooms_split, total=len(rooms_split)):
            ...
            # if 'stanford_indoor3d' in data_root:
            ...
            #     labels = np.zeros(room_data[:,-1].shape)
                
            #     for index, label in enumerate(classes_):
            #         if label in classes:
            #             labels[room_data[:,-1] == index] = classes.index(label)
            #         else:
            #             labels[room_data[:,-1] == index] = -1
            #     room_data[:, -1] = labels

            uniq_idx = voxelize(room_data[...,:3])
            room_data = room_data[uniq_idx]
    ...
    def __getitem__(self, idx):
        room_idx = self.room_idxs[idx]
        points, label = self.room_points[room_idx], self.room_labels[room_idx]

        coord, feat = points[:, 0:3], points[:, 3:6]
    ...
    def refresh(self):
        ...
        for room_name in tqdm(self.rooms_split, total=len(self.rooms_split)):
            ...
            # if 'stanford_indoor3d' in self.data_root:
            #     classes_ = ['ceiling', 'floor', 'wall', 'beam', 'column', 'window', 'door', 'table', 'chair', 'sofa', 'bookcase', 
            #                 'board', 'clutter']
        ...
        room_idxs = []
        for index, room_name in enumerate(self.rooms_split):
            room_idxs.extend([index] * int(round(sample_prob[index] * num_iter)))
        self.room_idxs = np.array(room_idxs)
